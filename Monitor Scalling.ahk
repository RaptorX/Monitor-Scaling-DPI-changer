...
; ApplySelectedScale(obj,info)
; {
;     Saved := MyGui.Submit(false)
;     setDPI(GetDisplayPath(Saved.Display), DPI_ENUM[Saved.Scale])
; }

; onSelectDisplay(obj,info)
; {
;     ControlSend("{backspace}",myPrefix)
; }

; GetCurrentDisplayPath()
; {
;     for k, DisplayName in Displays.array
;     {
;         RegExMatch(DIsplayName,"(\d+):",&DisplayNumber)
;         if DisplayNumber[1] = getCurrentDisplayNumberByMouse()
;         {
;             return DisplayPath := GetDisplayPath(regexreplace(DisplayName,"\d+:\s"))
;         }
;     }
; }
...
; GetDisplayPath(Display)
; {
;     for k, Path in GetAllDisplay()
;         if InStr(Path,Display)
;             return path
; }
...
;     end:=DISPLAYCONFIG_PATH_INFO_arr.Size
;     DISPLAYCONFIG_TARGET_DEVICE_NAME:=Buffer(420)
;     NumPut("Int",4,DISPLAYCONFIG_TARGET_DEVICE_NAME,0) ;2=DISPLAYCONFIG_DEVICE_INFO_GET_TARGET_NAME
;     NumPut("Uint",DISPLAYCONFIG_TARGET_DEVICE_NAME.Size,DISPLAYCONFIG_TARGET_DEVICE_NAME,4)
;     finalStr:=""
;     while (i_ < end) {
;         adapterID:=NumGet(DISPLAYCONFIG_PATH_INFO_arr, i_+0, "Uint64")
...
;         NumPut("Uint",targetID,DISPLAYCONFIG_TARGET_DEVICE_NAME,16)
;         DllCall("DisplayConfigGetDeviceInfo", "Ptr",DISPLAYCONFIG_TARGET_DEVICE_NAME)
;         DSP.push(StrGet(DISPLAYCONFIG_TARGET_DEVICE_NAME.Ptr + 164, "UTF-16"))
;         i_+=72
;     }
;     return DSP
; }

; GetScaleFactor(DSplyScaleID)
; {
;     Display := Map()
;     ; RegWrite, REG_DWORD, HKEY_CURRENT_USER, Control Panel\Desktop\PerMonitorSettings\RTK1D1A16843009_1E_07DF_17^E20410D943FA76F3E7FB5848A4A3085C, DpiValue, 1
;     ;Loop Reg, "HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\GraphicsDrivers\ScaleFactors", "KVR"
;     Loop Reg, "HKEY_CURRENT_USER\Control Panel\Desktop\PerMonitorSettings", "KVR"
;     {
;         if A_LoopRegName = "DpiValue"
;             if InStr(A_LoopRegKey,"\" DSplyScaleID)
;                 return A_LoopRegKey
;     }
; }
...
;     {
;         if RegExMatch(Monitor.DeviceID,"\w+\\(\w+)\\", &u)
;         {
;             DSplyScaleID.DefineProp( a_index ": " u[1] , {Value:""} )
;             Array.Push(a_index ": " u[1])
;         }
;         DSplyScaleID.DefineProp( 0 ": ActiveDisplay" , {Value:""} )
;         DSplyScaleID.Array := Array
;         ;u := RegRead("HKLM\SYSTEM\CurrentControlSet\Enum\" U "\Device Parameters", "EDID")
;     }
;     return DSplyScaleID
; }
